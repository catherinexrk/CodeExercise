...
int func(){
	int n,bagWeight;//n为物品的数量，bagWeight为背包存放的最大容量
	cin>>n>>bagWeight;
	
	vector<int> weight(n,0);//存储每个物品的所占容量
	vector<int> value(n,0);//记录每一个物品的对应价值
	...
	//dp[i][j]表示背包容量为j的情况下，从下标[0,i]的物品中任意选取，能达到的最大总价值
	vector<vector<int>> dp(n,vector<int>(bagWeight+1,0));
